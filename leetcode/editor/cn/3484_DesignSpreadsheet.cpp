...
//The cell reference is provided in the format "AX" (e.g., "A1", "B10"), where the 
//letter represents the column (from 'A' to 'Z') and the number represents a 1-
//indexed row. 
...
//
// 
// Input: ["Spreadsheet", "getValue", "setCell", "getValue", "setCell", 
//"getValue", "resetCell", "getValue"] [[3], ["=5+7"], ["A1", 10], ["=A1+6"], ["B2", 15], 
//["=A1+B2"], ["A1"], ["=A1+B2"]] 
// 
//
// Output: [null, 12, null, 16, null, 25, null, 15] 
//
// Explanation Spreadsheet spreadsheet = new Spreadsheet(3); // Initializes a 
...
//cell references or non-negative integers with values less than or equal to 10⁵. 
// Each cell reference consists of a capital letter from 'A' to 'Z' followed by 
//a row number between 1 and rows. 
// At most 10⁴ calls will be made in total to setCell, resetCell, and getValue. 
//
// 
//
// Related Topics 设计 数组 哈希表 字符串 矩阵 👍 18 👎 0
...
public:
    Spreadsheet(int rows) {
        sheet.assign(rows + 1, vector<int>(26, 0)); // 多留一行内存方便从一开始写索引
    }
    ...
    int getValue(string formula) {
        ...
        string X, Y;
        auto X_plus_Y = formula.substr(1);
        for (int i = 0; i < X_plus_Y.size(); i++) {
            if (X_plus_Y[i] == '+') {
                X = X_plus_Y.substr(0, i);
                // cout << "X: " << X << endl;
                Y = X_plus_Y.substr(i + 1);
                // cout << "Y: " << Y << endl;
                break;
            }
        }
        ...
        // 获取操作数值
        auto getOperandValue = [this](const string& op) -> int {
            return isalpha(op[0]) ? get(op) : stoi(op);
        };
