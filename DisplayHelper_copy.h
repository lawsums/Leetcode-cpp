...
namespace display {
    ...
    namespace detail {
        ...
        void display_impl(const std::tuple<Args...>& tuple, std::ostream& os) {
            display_tuple_impl(tuple, os, std::index_sequence_for<Args...>{});
        }

        // 显示迭代器容器
        template<typename Iterable>
        auto display_impl(const Iterable& iterable, std::ostream& os)
                -> std::enable_if_t<detail::is_iterable<Iterable>::value &&
                                    !detail::is_pair<Iterable>::value &&
                                    !detail::is_tuple<Iterable>::value>
        {
            os << "[";
            bool first = true;
